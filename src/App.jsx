   ...
   const handleGenerate = async (isUpdate = false) => {
    ...
    const prompt = `
        ...
        2.  For 'legalReference', you MUST follow this **three-step internal process**:
            **Step A: Identify Legal Concepts and Keywords used in the issue or complaint.** First, analyze the 'User-Provided Scenario' and identify the core legal concepts involved and keywords relevant to the complaint or issue (e.g., 'pay equity,' 'at-will employment,' 'constructive discharge,' 'harassment.')
            **Step B: Find a School-Specific Case.** Second, find a real, verifiable court case from a **K-12 public or independent school context** that deals with the concepts and keywords from Step A. Your search should prioritize sources like Justia, Casetext, Westlaw, FindLaw, Fastcase, CourtListener, and the Legal Information Institute, and relevant verified case law found on search engines, and also compare them to relevant state and federal laws. This ensures compliance with state and federal laws and best practices. Do not provide case law references that do not match the exact complaint or issue keywords and concept.
            **Step C: Justify the Selection.** Third, in your final output, provide the case name and a concise explanation that **explicitly connects the legal principle with the complaint or issue** from the case to the user's specific scenario or complaint, including the keywords in the complaint or issue, as the basis for the referenced case law.
            **Fallback Rule:** If, and only if, you cannot find a relevant school-specific case after this process, you MUST state, "No direct K-12 case law was found for this specific issue." Then, cite the controlling federal or state **statute** (e.g., Title VII, Equal Pay Act) and explain its application to schools. Providing irrelevant or non-school-related case law is an absolute failure, so do not step outside the complaint or issue, including keywords.
